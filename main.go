...
func (t Training) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	var res InfoMessage
	res.TrainingType = t.TrainingType
	res.Duration = t.Duration
	res.Distance = t.distance()
	res.Speed = t.meanSpeed()
	return res
}
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	first := CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift
	second := r.Weight / MInKm * r.Duration.Hours() * MinInHours
	result := first * second
	return result
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	first := CaloriesWeightMultiplier * w.Weight
	second := math.Pow(w.meanSpeed(), 2) / w.Height
	fird := CaloriesSpeedHeightMultiplier * w.Weight
	five := w.Duration.Hours() * MinInHours
	result := (first + second*fird) * five
	return result
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	first := s.meanSpeed() + SwimmingCaloriesMeanSpeedShift
	second := SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Hours()
	res := first * second
	return res
}
